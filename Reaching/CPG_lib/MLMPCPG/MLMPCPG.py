...
#global k1,l1,k2,l2,k3,l3,k4,l4,k,l,V_F,q_F,kk1,ll1,kk2,ll2,kk3,ll3,kk4,ll4,kk,ll,V_E,q_E
...
class mlmpCPG(object):
    ...
    #########################
    def fUpdateLocomotionNetwork2(self,myT,JointPos):
    ...
    def fUpdateLocomotionNetworkPF(self):
        ...
        self.PF.E.o = 1/(1+np.exp(self.PF.E.alpha*self.PF.E.alpha_MLR*((self.PF.E.theta+self.PF.E.theta_MLR)-self.PF.E.i)))
        self.PF.F.o = 1/(1+np.exp(self.PF.F.alpha*self.PF.F.alpha_MLR*((self.PF.F.theta+self.PF.F.theta_MLR)-self.PF.F.i)))
        return
    #########################
    def fUpdateLocomotionNetworkMN(self):
        ...
        return
    #########################
    def fUpdateLocomotionNetworkMN2(self):
...
#########################
def fUpdateGSN(FsrLeft,FsrRight,c,a):
        return (1.0/(1+math.exp(a*(c-FsrLeft))) , 1.0/(1+math.exp(a*(c-FsrRight))))
...
#########################
"""
def fPrintMLMPcpg(CPG):

    print('\n')
    print('Number of CPGs =', len(CPG))
    for i in range(0, len(CPG)):
        print('----- CPG: ',i, ' -----')
        print('-1- Description :', CPG[i].description)
        print('\n')
        print('-2- joint name :', CPG[i].joint.name)
        print('-3- joint maxJointAng :', CPG[i].joint.maxJointAng)
        print('-4- joint minJointAng :', CPG[i].joint.minJointAng)
        print('-5- joint init_motor_pos :', CPG[i].joint.init_motor_pos)
        print('-6- joint gain_joint_motor :', CPG[i].joint.gain_joint_motor)
        print('-7- joint motor_signal :', CPG[i].joint.joint_motor_signal)
        print('\n')
        print('-8- RG E Es:', CPG[i].RG.E.Es)
        print('-9- RG E A_f:', CPG[i].RG.E.A_f)
        print('-10- RG E InjCurrent_value:', CPG[i].RG.E.InjCurrent_value)
        print('-11- RG E InjCurrent_MultiplicationFactor:', CPG[i].RG.E.InjCurrent_MultiplicationFactor)
        print('-12- RG E sigma_f:', CPG[i].RG.E.sigma_f)
        print('-13- RG E sigma_s:', CPG[i].RG.E.sigma_s)
        print('-14- RG E tau_m:', CPG[i].RG.E.tau_m)
        print('-15- RG E tau_s:', CPG[i].RG.E.tau_s)
        print('-16- RG E V_0:', CPG[i].RG.E.V_0)
        print('-17- RG E q_0:', CPG[i].RG.E.q_0)
        print('-18- RG E V:', CPG[i].RG.E.V)
        print('-19- RG E q:', CPG[i].RG.E.q)
        print('-20- RG E out:', CPG[i].RG.E.out)
        print('\n')
        print('-21- RG F Es:', CPG[i].RG.F.Es)
        print('-22- RG F A_f:', CPG[i].RG.F.A_f)
        print('-23- RG F InjCurrent_value:', CPG[i].RG.F.InjCurrent_value)
        print('-24- RG F InjCurrent_MultiplicationFactor:', CPG[i].RG.F.InjCurrent_MultiplicationFactor)
        print('-25- RG F sigma_f:', CPG[i].RG.F.sigma_f)
        print('-26- RG F sigma_s:', CPG[i].RG.F.sigma_s)
        print('-27- RG F tau_m:', CPG[i].RG.F.tau_m)
        print('-28- RG F tau_s:', CPG[i].RG.F.tau_s)
        print('-29- RG F V_0:', CPG[i].RG.F.V_0)
        print('-30- RG F q_0:', CPG[i].RG.F.q_0)
        print('-31- RG F V:', CPG[i].RG.F.V)
        print('-32- RG F q:', CPG[i].RG.F.q)
        print('-33- RG F out:', CPG[i].RG.F.out)
        print('\n')
        print('-34- PF E alpha:', CPG[i].PF.E.alpha)
        print('-35- PF E theta:', CPG[i].PF.E.theta)
        print('-36- PF E alpha_MLR:', CPG[i].PF.E.alpha_MLR)
        print('-37- PF E theta_MLR:', CPG[i].PF.E.theta_MLR)
        print('-38- PF E i:', CPG[i].PF.E.i)
        print('-39- PF E o:', CPG[i].PF.E.o)
        print('\n')
        print('-40- PF F alpha:', CPG[i].PF.F.alpha)
        print('-41- PF F theta:', CPG[i].PF.F.theta)
        print('-42- PF F alpha_MLR:', CPG[i].PF.F.alpha_MLR)
        print('-43- PF F theta_MLR:', CPG[i].PF.F.theta_MLR)
        print('-44- PF F i:', CPG[i].PF.F.i)
        print('-45- PF F o:', CPG[i].PF.F.o)
        print('\n')
        print('-46- MN E alpha :', CPG[i].MN.E.alpha)
        print('-47- MN E theta:', CPG[i].MN.E.theta)
        print('-48- MN E i:', CPG[i].MN.E.i)
        print('-49- MN E o:', CPG[i].MN.E.o)
        print('\n')
        print('-50- MN F alpha :', CPG[i].MN.F.alpha)
        print('-51- MN F theta:', CPG[i].MN.F.theta)
        print('-52- MN F i:', CPG[i].MN.F.i)
        print('-53- MN F o:', CPG[i].MN.F.o)
        print('\n')
        print('-54- W_F_RG2PF:', CPG[i].W_F_RG2PF)
        print('-55- W_E_RG2PF:', CPG[i].W_E_RG2PF)
        print('-56- W_F_PF2MN:', CPG[i].W_F_PF2MN)
        print('-57- W_E_PF2MN:', CPG[i].W_E_PF2MN)
