...
/////////////////////////////////////////////////////////////////////////////
// proj8: Injected_current -> Injected_current with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct8 : LILMatrix<int, int> {
    ProjStruct8() : LILMatrix<int, int>( 80, 80) {
    }
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct8::init_projection() - this = " << this << std::endl;
    #endif
    ...
    void compute_psp() {
        ...
        std::cout << "    ProjStruct8::compute_psp()" << std::endl;
    #endif
        double sum;
        ...
        if (_transmission && pop8._active){
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    sum += pop8.r[pre_rank[i][j]]*w[i][j] ;
                }
                pop8._sum_inh[post_rank[i]] += sum;
            }
    ...
    void update_synapse() {
    #ifdef _TRACE_SIMULATION_STEPS
        std::cout << "    ProjStruct8::update_synapse()" << std::endl;
    #endif
    ...
    std::vector<std::vector<double>> get_local_attribute_all_double(std::string name) {
        ...
        // should not happen
        std::cerr << "ProjStruct8::get_local_attribute_all_double: " << name << " not found" << std::endl;
    ...
    std::vector<double> get_local_attribute_row_double(std::string name, int rk_post) {
        ...
        // should not happen
        std::cerr << "ProjStruct8::get_local_attribute_row_double: " << name << " not found" << std::endl;
    ...
    double get_local_attribute_double(std::string name, int rk_post, int rk_pre) {
        ...
        // should not happen
        std::cerr << "ProjStruct8::get_local_attribute: " << name << " not found" << std::endl;
        return 0.0;
    }
    ...
    void clear() {
    #ifdef _DEBUG
        std::cout << "ProjStruct8::clear() - this = " << this << std::endl;
    #endif
